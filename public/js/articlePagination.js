{
    ...
    const root = document.getElementById("article-affichage");
        fillTable(root,"/articles?page=1&limit=10");
    ...
    async function fillTable(root,url){
        ...
        for(article of data){
            ...
            const user =await( await fetch(`/users/${article.UserId}`) ).json();
            htmlTemp += `
        ...
        <p class="post-meta">Posted by
          ...
          <B>Tags : </B>
        `
        const tags =await( await fetch(`/articles/${article.id}/tags`) ).json();  
            //boucle 2 :
            for(tag of tags){
